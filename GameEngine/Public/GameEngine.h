...
public:
	...
	// 게엠 엔진이 소유하는 윈도우를 반환합니다.
	// 이때, 반환된 윈도우는 변경 가능합니다.
	//
	// @return - 윈도우의 참조자를 반환합니다.
	static class GameWindow& GetGameWindow();


	// 게엠 엔진이 소유하는 입력 시스템을 반환합니다.
	// 이때, 반환된 입력 시스템은 변경 가능합니다.
	// 
	// @return - 입력 시스템의 참조자를 반환합니다.
	static class GameInput& GetGameInput();


	// 게엠 엔진이 소유하는 렌더러를 반환합니다.
	// 이때, 반환된 렌더러는 변경 가능합니다.
	// 
	// @return - 렌더러의 참조자를 반환합니다.
	static class GameRenderer& GetGameRenderer();


	// 게임 실행 파일 경로를 반환합니다.
	// 
	// @return - 게임 실행 파일 경로를 반환합니다.
	static const std::string& GetExecuteDirectory() { return ExecuteDirectory; }


	// 게임 프로젝트의 루트 디렉토리 경로를 반환합니다.
	//
	// @return - 게임 프로젝트의 루트 디렉토리 경로를 반환합니다.
	static const std::string& GetProjectRootDirectory() {return ProjectRootDirectory; }


	// 게임 엔진 디렉토리 경로를 반환합니다.
	//
	// @return - 게임 엔진 디렉토리 경로를 반환합니다.
	static const std::string& GetEngineDirectory() { return EngineDirectory; }


	// 게임 리소스 디렉토리 경로를 반환합니다.
	//
	// @return - 게임 리소스 디렉토리 경로를 반환합니다.
	static const std::string& GetResourceDirectory() { return ResourceDirectory; }
...
private:
	...
	// 게엠 엔진이 소유하는 입력 시스템입니다.
	static std::unique_ptr<class GameInput> Input;


	// 게엠 엔진이 소유하고 있는 렌더러 입니다.
