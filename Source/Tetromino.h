...
public:
	// 생성자입니다.
	//
	// @param InAbsolutePosition - 테트로미노의 보드 상 왼쪽 상단 좌표입니다.
	// @param InShape - 테트로미노의 모양입니다.
	// @param InColor - 테트로미노 블럭의 색상입니다.
	Tetromino(const Vec2i& InAbsolutePosition, const EShape& InShape, const LinearColor& InColor);
	...
	// 테트로미노의 보드 상 위치를 반환합니다.
	//
	// @return - 테트로미노의 보드 상 위치를 반환합니다.
	Vec2i GetAbsolutePosition() noexcept { return AbsolutePosition; }


	// 테트로미노 블럭들의 상대 위치를 반환합니다.
	//
	// @return - 테트로미노 블럭들의 상대 위치를 반환합니다..
	const std::vector<Vec2i>& GetRelativePositions() noexcept { return RelativePositions; }
	...
	void Draw(const Vec2i& InWindowPosition, int32_t InBlockSize, const LinearColor& InOutlineColor);
...
private:
	...
	void GenerateRelativePositions(const EShape& InShape);
