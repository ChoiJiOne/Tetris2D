...
namespace Game
{
	...
	public:
		/**
		 * 괸리중인 리소스를 정리합니다.
		 * @note 프로그램 종료 이전에 반드시 호출되어야 합니다.
		 */
		static void Cleanup();
		 ...
		 * @param InHashKey - 생성할 텍스처 리소스의 해쉬 키값입니다.
		 ...
		 */
		static const Texture2D& CreateTexture2D(SDL_Renderer* InRenderer, const std::size_t& InHashKey, const std::string& InPath);
		 ...
		 * 
		 * @param InRenderer - SDL 렌더러의 포인터입니다.
		 * @param InHashKey - 생성할 트루 타입 폰트 리소스의 해쉬 키값입니다.
		 * @param InPath - 트루 타입 폰트 리소스의 경로입니다.
		 ...
		 * @throws 
		 * 키 값이 충돌할 경우, C++ 표준 예외를 던집니다.
		 * 트루 타입 폰트 리소스 생성 실패 시, C++ 표준 예외를 던집니다.
		 */
		 ...
		 */
		static const Texture2D& GetTexture2D(const std::size_t& InHashKey);


		/**
		 * 리소스 메너저가 괸리하는 폰트를 반환합니다.
		 * 
	...
	private:
		...
		static std::unordered_map<std::size_t, std::shared_ptr<Texture2D>> TextureCache;
