...
HRESULT IShader::CompileShaderFromFile(const std::wstring& SourcePath, const std::string& EntryPoint, const std::string& ShaderModel, ID3DBlob** Blob)
{
	HRESULT HR = S_OK;
...
HRESULT IShader::CreateVertexShaderFromFile(ID3D11Device* Device, const std::wstring& SourcePath)
{
	SAFE_RELEASE(VertexShaderSource_);
	SAFE_RELEASE(VertexShader_);
...
HRESULT IShader::CreatePixelShaderFromFile(ID3D11Device* Device, const std::wstring& SourcePath)
{
	SAFE_RELEASE(PixelShaderSource_);
	SAFE_RELEASE(PixelShader_);
...
HRESULT IShader::CreateInputLayout(ID3D11Device* Device, const std::vector<D3D11_INPUT_ELEMENT_DESC>& InputLayoutElements)
{
	SAFE_RELEASE(InputLayout_);
...
HRESULT IShader::CreateIndexBuffer(ID3D11Device* Device, const std::vector<uint32_t>& Indices, ID3D11Buffer** IndexBuffer)
{
...
HRESULT IShader::CreateLinearTextureSampler(ID3D11Device* Device, ID3D11SamplerState** LinearTextureSampler)
{
